...
export default function OfficerDashboard() {
  ...
  useEffect(() => {
    ...
    Promise.all([assignmentApi.officerDashboard(), assignmentApi.officerPerformance(rangeDays)])
      .then(([dashboardResult, performanceResult]) => {
        if (!active) return;
    ...
    <section className="page-wrap">
      ...
      <section className="surface section-gap">
        <div className="surface-header">
          <div><h3>Personal performance</h3><p className="muted">Calculated from your existing assignment and verification records.</p></div>
          <select className="input" style={{ width: 160 }} value={rangeDays} onChange={(event) => setRangeDays(Number(event.target.value))}>
