...
public sealed partial class ChatbotService : IChatbotService
{
    ...
    private readonly IPythonChatbotClient _pythonChatbot;

    public ChatbotService(
        IChatRepository chatRepository,
        IUnitOfWork unitOfWork,
        ICurrentUserService currentUser,
        IPythonChatbotClient pythonChatbot)
    {
        _chatRepository = chatRepository;
        _unitOfWork = unitOfWork;
        _currentUser = currentUser;
        _pythonChatbot = pythonChatbot;
    }
    ...
    public async Task<ChatReplyResponse> SendMessageAsync(ChatMessageRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var pythonReply = await _pythonChatbot.GenerateAsync(messageText, session.SessionId, RequireUserId(), cancellationToken);
        var result = pythonReply is null
            ? await GenerateReplyAsync(messageText, cancellationToken)
            : new BotResult("PythonAssistant", pythonReply.Confidence, pythonReply.Message, null, null);
