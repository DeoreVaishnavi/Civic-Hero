...
export default function UserManagement() {
  ...
  return <section className="p-6 lg:p-10">
    <div className="flex flex-wrap items-end justify-between gap-4"><div><h2 className="text-3xl font-black text-slate-900">User management</h2><p className="mt-1 text-slate-400">Create Citizens, control access, change email, preserve account history and perform audited soft deletion.</p></div><span className="text-sm text-slate-400">{pageInfo.totalCount || 0} users</span></div>

    <form onSubmit={createCitizen} className="cv-card mt-6 rounded-2xl border border-emerald-300 bg-emerald-50 p-5">
      <h3 className="text-lg font-black text-emerald-900">Create Citizen account</h3>
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-3"><label className="flex items-center gap-2 text-sm text-slate-700"><input type="checkbox" checked={createForm.markEmailVerified} onChange={(e) => setCreateForm({ ...createForm, markEmailVerified: e.target.checked })}/>Mark email verified after administrative identity check</label><button disabled={working} className="cv-btn-success disabled:opacity-100">Create Citizen</button></div>
    </form>
