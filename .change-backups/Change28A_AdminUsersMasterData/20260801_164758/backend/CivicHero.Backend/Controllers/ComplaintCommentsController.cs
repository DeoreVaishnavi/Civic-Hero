...
public sealed class ComplaintCommentsController : ControllerBase
{
    ...
    public async Task<IActionResult> Update(long complaintId, long commentId, [FromBody] UpdateComplaintCommentRequest request, CancellationToken cancellationToken) =>
        Ok(new { success = true, message = "Comment updated.", data = await _service.UpdateAsync(complaintId, commentId, request, cancellationToken) });

    [HttpPost("{commentId:long}/report")]
    [Authorize(Policy = PermissionConstants.CitizenOnly)]
    public async Task<IActionResult> Report(long complaintId, long commentId, [FromBody] ReportComplaintCommentRequest request, CancellationToken cancellationToken) =>
        Ok(new { success = true, message = "Comment reported for safety review.", data = await _service.ReportAsync(complaintId, commentId, request, cancellationToken) });
