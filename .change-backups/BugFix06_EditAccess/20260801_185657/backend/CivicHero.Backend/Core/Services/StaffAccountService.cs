...
public sealed class StaffAccountService : IStaffAccountService
{
    ...
    public async Task<StaffAccountMetadataDto> GetMetadataAsync(
        CancellationToken cancellationToken = default)
    {
        ...
        var departments = await _db.Departments.AsNoTracking()
            ...
            .Select(item => new StaffDepartmentOptionDto
            {
                Id = item.Id,
                Name = item.Name,
                Code = item.Code
        ...
        var departmentIds = departments.Select(item => item.Id).ToList();
        List<StaffWardOptionDto> wards;
        ...
        {
            wards = await _db.Wards.AsNoTracking()
                .Where(item => item.IsActive && departmentIds.Contains(item.DepartmentId))
                .OrderBy(item => item.Name)
                .Select(item => new StaffWardOptionDto
                {
                    Id = item.Id,
        ...
        return new StaffAccountMetadataDto
        {
