...
public sealed class ReopenDisputeValidator : AbstractValidator<ReopenDisputeRequest>
{
    public ReopenDisputeValidator()
    {
        RuleFor(x => x.Reason).NotEmpty().MinimumLength(10).MaximumLength(1500);
    }
}

public sealed class DisputeEvidenceRequestValidator : AbstractValidator<DisputeEvidenceRequest>
{
    public DisputeEvidenceRequestValidator()
    {
        RuleFor(x => x.TargetRole)
            .Must(value => new[] { "Citizen", "Officer" }.Contains(value, StringComparer.OrdinalIgnoreCase))
            .WithMessage("Evidence can be requested from the Citizen or assigned Officer.");
        RuleFor(x => x.Message).NotEmpty().MinimumLength(10).MaximumLength(1500);
        RuleFor(x => x.DueAt)
            .Must(value => !value.HasValue || (value.Value > DateTimeOffset.UtcNow && value.Value <= DateTimeOffset.UtcNow.AddDays(30)))
            .WithMessage("Evidence due date must be within the next 30 days.");
    }
}
