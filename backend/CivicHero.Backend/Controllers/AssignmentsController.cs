...
public sealed class AssignmentsController : ControllerBase
{
    ...
    [HttpPost("{complaintId:long}/transfer-request")]
    [Authorize(Policy = PermissionConstants.OfficerOnly)]
    public async Task<IActionResult> TransferRequest(long complaintId, [FromBody] TransferAssignmentRequest request, CancellationToken cancellationToken)
    {
        await ValidateAsync(request, cancellationToken);
        return OkEnvelope("Transfer request submitted to the supervisor queue.", await _service.RequestTransferAsync(complaintId, request, cancellationToken));
    }
    ...
    [HttpPost("{complaintId:long}/progress-evidence")]
    [Authorize(Policy = PermissionConstants.OfficerOnly)]
    [Consumes("multipart/form-data")]
    [RequestSizeLimit(55 * 1024 * 1024)]
    public async Task<IActionResult> ProgressEvidence(long complaintId, [FromForm] AddProgressEvidenceRequest request, CancellationToken cancellationToken)
    {
        await ValidateAsync(request, cancellationToken);
        return OkEnvelope("Progress update and evidence recorded.", await _service.AddProgressWithEvidenceAsync(complaintId, request, cancellationToken));
    }

    [HttpPost("{complaintId:long}/request-citizen-information")]
    [Authorize(Policy = PermissionConstants.OfficerOnly)]
    public async Task<IActionResult> RequestCitizenInformation(long complaintId, [FromBody] OfficerCitizenInformationRequest request, CancellationToken cancellationToken)
    {
        await ValidateAsync(request, cancellationToken);
        return OkEnvelope("Private information request sent to the citizen.", await _service.RequestCitizenInformationAsync(complaintId, request, cancellationToken));
    }
    ...
    [HttpGet("dashboard/officer/performance")]
    [Authorize(Policy = PermissionConstants.OfficerOnly)]
    public async Task<IActionResult> OfficerPerformance([FromQuery] int days = 90, CancellationToken cancellationToken = default) =>
        OkEnvelope("Officer performance loaded.", await _service.GetOfficerPerformanceAsync(days, cancellationToken));

    [HttpGet("map/officer")]
    [Authorize(Policy = PermissionConstants.OfficerOnly)]
    public async Task<IActionResult> OfficerMap(CancellationToken cancellationToken) =>
        OkEnvelope("Officer work map loaded.", await _service.GetOfficerMapAsync(cancellationToken));
