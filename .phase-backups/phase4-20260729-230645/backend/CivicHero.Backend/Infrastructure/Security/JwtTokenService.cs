...
public sealed class JwtTokenService : IJwtTokenService
{
    ...
    public JwtTokenService(IOptions<JwtOptions> options)
    {
        ...
        if (Encoding.UTF8.GetByteCount(_options.SecretKey) < 32 ||
            _options.SecretKey.StartsWith("CHANGE_THIS", StringComparison.OrdinalIgnoreCase))
        {
            throw new InvalidOperationException(
                "Jwt:SecretKey must contain at least 32 bytes. Configure it with .NET user-secrets.");
        }
    }
