  ...
  return <section className="space-y-6 p-6">
    ...
    <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <span className="rounded-full bg-sky-400/10 px-3 py-1 text-xs font-bold text-sky-200">{sessionResult.total ?? 0} matching sessions</span>
      </div>
      ...
      <form onSubmit={applySessionFilters} className="mt-4 grid gap-3 md:grid-cols-[minmax(0,1fr)_220px_auto]">
        <input
          ...
          className="rounded-xl border border-white/10 bg-slate-950/60 px-4 py-3 text-sm text-white outline-none focus:border-sky-400"
        />
        <select
        ...
        <div className="flex gap-2">
          <button type="submit" className="rounded-xl bg-sky-500 px-4 py-3 text-sm font-bold text-white hover:bg-sky-400">Apply</button>
          <button type="button" onClick={clearSessionFilters} className="rounded-xl border border-white/15 px-4 py-3 text-sm font-bold text-white hover:bg-white/10">Clear</button>
        </div>
      </form>
              ...
              : sessionResult.sessions.map((item) => <tr key={item.sessionId}>
                <td className="p-3">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {item.isCurrentActorSession && <span className="rounded-full bg-amber-400/15 px-2 py-0.5 text-[10px] font-bold text-amber-200">Current admin session</span>}
                  </div>
                ...
                <td className="p-3"><button disabled={busyId === `session-${item.sessionId}`} onClick={() => revokeSelectedSession(item)} className="rounded-lg bg-rose-500/20 px-3 py-2 text-xs font-bold text-rose-200 disabled:opacity-50">{busyId === `session-${item.sessionId}` ? 'Revoking…' : 'Revoke selected'}</button></td>
              </tr>)}
          </tbody>
        </table>
      </div>
