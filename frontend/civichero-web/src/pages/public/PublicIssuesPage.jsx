...
export default function PublicIssuesPage() {
  ...
  useEffect(() => {
    if (isAuthenticated && isCitizen) {
      complaintApi.followingIds()
        .then((data) => setFollowedIds(new Set(data?.complaintIds || [])))
        .catch(() => {});
    } else {
  ...
  const toggleFollow = async (item) => {
    ...
    if (!isCitizen) {
      setError('Only a Citizen account can follow community complaints.');
      return;
    }
    ...
    try {
      ...
      const response = isFollowing
        ...
        : await complaintApi.follow(item.id);
      setFollowedIds((current) => {
        const next = new Set(current);
        if (response?.isFollowing) next.add(item.id); else next.delete(item.id);
        return next;
      });
      setMessage(response?.isFollowing
        ? 'Complaint added to your following feed.'
        : 'Complaint removed from your following feed.');
    } catch (reason) {
      setError(reason.message || 'Unable to update following.');
    } finally {
      setFollowingId(null);
    }
  };
                    ...
                    <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-5">
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        <button
                          ...
                          disabled={followingId === item.id}
                          className={`rounded-xl border px-4 py-2.5 text-xs font-black transition disabled:cursor-wait disabled:opacity-60 ${followedIds.has(item.id) ? 'border-violet-200 bg-violet-50 text-violet-700 hover:bg-violet-100' : 'border-slate-200 text-slate-600 hover:border-violet-200 hover:bg-violet-50 hover:text-violet-700'}`}
                        >
                          {followingId === item.id ? 'Updating…' : followedIds.has(item.id) ? '✓ Following' : '☆ Follow updates'}
                        </button>
                        <button
                          type="button"
