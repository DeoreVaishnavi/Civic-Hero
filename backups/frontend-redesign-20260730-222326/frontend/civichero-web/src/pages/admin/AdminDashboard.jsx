  ...
  return (
    <section className="page-wrap">
      ...
      <div className="stat-grid six section-gap">
        <Stat icon="▦" label="Departments" value={stats?.activeDepartments ?? '—'} hint="Active master records" />
        <Stat icon="⌖" label="Wards" value={stats?.activeWards ?? '—'} hint="Geographic areas" />
        <Stat icon="≡" label="Categories" value={stats?.activeCategories ?? '—'} hint="Complaint categories" tone="violet" />
        <Stat icon="○" label="Active users" value={stats?.activeUsers ?? '—'} hint="All enabled accounts" tone="green" />
        <Stat icon="!" label="Open complaints" value={stats?.openComplaints ?? '—'} hint="Requires processing" tone="amber" />
        <Stat icon="◇" label="Audit events today" value={stats?.auditEventsToday ?? '—'} hint="Traceable actions" tone="red" />
      </div>
      ...
      <section className="surface section-gap">
        ...
        <div className="quick-actions" style={{ padding: 15 }}>
          <Quick to="/admin/users" icon="○" title="User and officer management" text="Activate, deactivate and assign roles" />
          <Quick to="/admin/ai-review" icon="◎" title="AI review queue" text="Review suspicious complaints" />
          <Quick to="/admin/audit-logs" icon="◇" title="Audit logs" text="Inspect secure action history" />
          <Quick to="/admin/reports" icon="↗" title="Generate reports" text="Export operational evidence" />
        </div>
      </section>

      <div className="dashboard-grid equal section-gap">
        <section className="surface"><div className="surface-header"><h3>Department work tracking</h3><Link to="/admin/analytics" className="button ghost small">View performance →</Link></div><div className="civic-table-wrap"><table className="civic-table" style={{ minWidth: 480 }}><thead><tr><th>Department</th><th>Workload</th><th>Completed</th><th>Pending</th></tr></thead><tbody><tr><td><strong>Roads</strong></td><td>Live analytics</td><td>See dashboard</td><td>Open queue</td></tr><tr><td><strong>Sanitation</strong></td><td>Live analytics</td><td>See dashboard</td><td>Open queue</td></tr><tr><td><strong>Electrical</strong></td><td>Live analytics</td><td>See dashboard</td><td>Open queue</td></tr></tbody></table></div></section>
        <section className="surface"><div className="surface-header"><h3>System alerts</h3><Link to="/admin/system-health" className="button ghost small">Open health centre →</Link></div><div className="surface-body notification-list"><Notice icon="●" title="Database and storage" text="Use system health to verify AWS RDS and Amazon S3 dependencies." /><Notice icon="♢" title="Notification channels" text="Review SignalR, email, SMS and queue delivery status." /><Notice icon="◇" title="Security controls" text="Inspect lockouts, permissions and audit activity." /></div></section>
      </div>
    </section>
  );
}
