    ...
    <section className="p-6 lg:p-10">
      <div className="mx-auto max-w-4xl">
        ...
        <form onSubmit={submit} className="space-y-6 rounded-3xl border border-white/10 bg-white/[0.04] p-6 lg:p-8">
          ...
          <div className="rounded-2xl border border-amber-400/25 bg-amber-400/5 p-5"><label className="flex items-start gap-3"><input type="checkbox" name="possibleEmergency" checked={form.possibleEmergency} onChange={change} className="mt-1" /><span><strong className="text-amber-100">Possible emergency requiring urgent official review</strong><span className="mt-1 block text-sm text-amber-200/70">This temporarily raises attention but only a Supervisor or Admin can confirm High/Critical priority.</span></span></label>{form.possibleEmergency && <textarea className="input mt-4 min-h-24" name="emergencyReason" value={form.emergencyReason} onChange={change} placeholder="Explain the immediate public-safety risk" required />}</div>
