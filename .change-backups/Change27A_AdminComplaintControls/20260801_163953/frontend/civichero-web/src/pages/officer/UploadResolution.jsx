...
export default function UploadResolution() {
  ...
  const previews = evidence.map((file) => ({ file, url: URL.createObjectURL(file) }));
  ...
  return (
    <section className="page-wrap narrow">
      ...
      <form onSubmit={submit}>
        ...
        <section className="surface form-section">
          <div className="surface-header" style={{ margin: '-19px -19px 19px' }}><h3>Upload after images</h3><span className="status-pill">1–5 images</span></div>
          <label className="upload-zone" style={{ display: 'block', cursor: 'pointer' }}><span className="feature-icon" style={{ margin: '0 auto' }}>▧</span><strong style={{ marginTop: 12 }}>Select completed-work photos</strong><p>Use clear photos from angles comparable to the original evidence.</p><input type="file" accept="image/jpeg,image/png,image/webp" multiple required hidden onChange={(e) => setEvidence(Array.from(e.target.files || []).slice(0,5))} /></label>
