...
const icons = { complaint: '▤', dispute: '!', reward: '🎁', security: '◇', system: '♢', assignment: '✓', verification: '◎' };
...
export default function NotificationsPage() {
  ...
  return (
    <section className="page-wrap">
      ...
      <div className="mt-5 flex gap-2 overflow-x-auto rounded-2xl border border-slate-200 bg-white p-2 shadow-sm">{filters.map((filter) => <button key={filter} type="button" onClick={() => setActiveFilter(filter)} className={`shrink-0 rounded-xl px-5 py-3 text-sm font-black transition ${activeFilter === filter ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'text-slate-600 hover:bg-slate-100'}`}>{filter}{filter === 'Complaints' && unreadCount > 0 ? ` ${unreadCount}` : ''}</button>)}</div>

      <div className="mt-6 grid gap-6 xl:grid-cols-[1.35fr_.65fr]">
        <div className="overflow-hidden rounded-[1.75rem] border border-slate-200 bg-white shadow-sm">
          <div className="border-b border-slate-200 px-6 py-5"><div className="flex items-center justify-between"><div><p className="text-xs font-black uppercase tracking-[.18em] text-blue-600">Notification list</p><h3 className="mt-1 text-xl font-black text-slate-900">{visibleItems.length} alerts</h3></div><span className={`rounded-full px-3 py-1.5 text-xs font-black ${connectionState === 'connected' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>● SignalR {connectionState}</span></div></div>
          <div className="divide-y divide-slate-100">{visibleItems.map((item) => {
            const type = String(item.type || 'system').toLowerCase();
            const icon = Object.entries(icons).find(([key]) => type.includes(key))?.[1] || '♢';
            return <article key={item.id} className={`group flex gap-4 p-5 transition hover:bg-blue-50/40 ${item.isRead ? 'bg-white' : 'bg-blue-50/60'}`}>
              <button type="button" onClick={() => read(item)} className="grid h-14 w-14 shrink-0 place-items-center rounded-2xl bg-blue-100 text-xl font-black text-blue-700 transition group-hover:scale-105">{icon}</button>
          ...
          })}{visibleItems.length === 0 && <div className="p-14 text-center"><div className="mx-auto grid h-16 w-16 place-items-center rounded-3xl bg-slate-100 text-3xl">🔔</div><h3 className="mt-5 text-lg font-black text-slate-900">No notifications found</h3><p className="mt-2 text-sm text-slate-500">There are no alerts matching this filter.</p></div>}</div>
        </div>
