...
public sealed class UserService : IUserService
{
    ...
    public async Task<UserProfileDto> ChangeRoleAsync(long requesterId, string requesterRole, long targetUserId, ChangeRoleRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var previous = new { Role = target.Role.ToString(), target.DepartmentId, target.WardId };
        target.Role = newRole;
        ...
        {
            UserId = requesterId,
            UserRole = actorRole.ToString(),
            Action = "USER_ROLE_CHANGED",
            EntityName = "User",
            EntityId = target.Id.ToString(),
            OldValuesJson = System.Text.Json.JsonSerializer.Serialize(previous),
            NewValuesJson = System.Text.Json.JsonSerializer.Serialize(new
            {
                Role = target.Role.ToString(), target.DepartmentId, target.WardId
            }),
