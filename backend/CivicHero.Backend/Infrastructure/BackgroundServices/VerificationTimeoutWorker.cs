...
public sealed class VerificationTimeoutWorker : BackgroundService
{
 ...
 protected override async Task ExecuteAsync(CancellationToken stoppingToken){await Task.Delay(TimeSpan.FromSeconds(20),stoppingToken);while(!stoppingToken.IsCancellationRequested){try{await Process(stoppingToken);}catch(Exception ex){_logger.LogError(ex,"Verification timeout processing failed.");}await Task.Delay(TimeSpan.FromMinutes(5),stoppingToken);}}
