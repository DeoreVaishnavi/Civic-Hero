...
export default function SupervisorAiReviewQueue() {
  ...
  const runForensics = async (item) => {
    setForensicsBusy(item.complaintId);
    setError('');
    try {
      const report = await aiApi.mediaForensics(item.complaintId);
      setForensicsReport(report);
      setForensicsComplaintId(item.complaintId);
    } catch (reason) {
      setError(reason?.errors?.join(' ') || reason?.message || 'Media forensics could not be completed.');
    } finally {
      setForensicsBusy(null);
    }
  };
    ...
    <div className="mt-7 space-y-5">
      ...
      {queue.map((item) => <article key={item.complaintId} className={card}>
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          ...
          <Action disabled={forensicsBusy === item.complaintId} onClick={() => runForensics(item)}>{forensicsBusy === item.complaintId ? 'Analysing media…' : 'Media forensics'}</Action>
