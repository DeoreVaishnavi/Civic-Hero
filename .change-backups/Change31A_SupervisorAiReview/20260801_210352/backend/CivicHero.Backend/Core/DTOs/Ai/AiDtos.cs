...
public sealed record AiReviewQueueItem(long ComplaintId, string Title, string CitizenName, string Status, string PredictedCategory, string PredictedPriority, string DuplicateStatus, decimal DuplicateScore, string FraudVerdict, decimal FraudScore, string Reasoning, DateTimeOffset AnalyzedAt);
public sealed record AiReviewDecisionRequest(string Decision, string? Notes, long? MergeIntoComplaintId = null);
