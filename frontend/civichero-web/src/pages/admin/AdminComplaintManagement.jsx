...
export default function AdminComplaintManagement() {
  ...
  const [filters, setFilters] = useState({ ...initialFilters, search: headerSearch });
  ...
  useEffect(() => { if (tab === 'complaints') load(); else loadClusters(); }, [filters.page, filters.search, filters.status, filters.priority, filters.category, filters.departmentId, filters.wardId, filters.includeArchived, filters.duplicateOnly, tab]);
  ...
  const submitSearch = (event) => { event.preventDefault(); setFilters((value) => ({ ...value, page: 1 })); };
