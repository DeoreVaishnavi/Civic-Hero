...
var user = await _db.Users
    .AsNoTracking()
    .FirstAsync(entity => entity.Id == userId, cancellationToken);        var points = await BalanceAsync(userId, cancellationToken);
        var closed = await _db.Complaints.AsNoTracking().CountAsync(entity => entity.CitizenId == userId && (entity.Status == ComplaintStatus.Closed || entity.Status == ComplaintStatus.ClosedAuto), cancellationToken);
if (points < 250)
{
...
<head>
    <meta charset="utf-8">
    <title>CivicHero Certificate</title>
    <style>
        body {
...
<body>
    <main class="certificate">
        <div class="seal">★</div>
        <h1>CivicHero</h1>
        <p>Certificate of Civic Contribution</p>

        <h2>{{System.Net.WebUtility.HtmlEncode(user.FullName)}}</h2>

        <p>
            is recognised for responsible civic participation,
            helping close <strong>{{closed}}</strong> civic issue(s)
            and earning <strong>{{points}}</strong> CivicHero points.
        </p>

        <small>
            Issued {{DateTimeOffset.UtcNow:dd MMMM yyyy}}
            · Certificate CH-{{userId:D6}}-{{points:D6}}
        </small>
    </main>
</body>
</html>
