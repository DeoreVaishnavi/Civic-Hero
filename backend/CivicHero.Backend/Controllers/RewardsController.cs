...
public sealed class RewardsController : ControllerBase
{
    ...
    [HttpGet("leaderboard")]
    [AllowAnonymous]
    public async Task<IActionResult> Leaderboard([FromQuery] int limit = 25, [FromQuery] LeaderboardTimeframe timeframe = LeaderboardTimeframe.AllTime, CancellationToken cancellationToken = default) =>
        OkEnvelope($"{timeframe} leaderboard loaded.", await _service.GetLeaderboardAsync(limit, timeframe, cancellationToken));
