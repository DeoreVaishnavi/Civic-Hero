...
public sealed class RateLimitingMiddleware
{
    ...
    {
        if (path.StartsWithSegments("/api/v1/anonymous-complaints")) return ("anonymous-reporting", options.AnonymousReporting);
        if (path.StartsWithSegments("/api/v1/auth/phone")) return ("phone-otp", options.PhoneOtp);
