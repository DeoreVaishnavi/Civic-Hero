...
public sealed class DisputesController : ControllerBase
{
 ...
 [HttpGet("{id:long}/history")]public async Task<IActionResult> History(long id,CancellationToken ct)=>Ok(new{success=true,message="Dispute decision history loaded.",data=await _service.HistoryAsync(id,ct)});
